...
uint8_t maxtrix_buffer[8] = {0x00, 0x00, 0xFC, 0x12, 0x12, 0x12, 0xFC, 0x00};
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
	...
	if(collum_counter >= 1000){
		...
		shift_counter++;
		if(shift_counter == 30){
			uint8_t temp = maxtrix_buffer[0];
			for(uint8_t i = 1; i < 8; i++){
				maxtrix_buffer[i - 1] = maxtrix_buffer[i];
			}
			maxtrix_buffer[7] = temp;
			shift_counter = 0;
		}
		collum_counter = 0;
	}
...
/* USER CODE BEGIN 4 */
void updateClockBuffer(void){
	led_buffer[0] = hour / 10; //extract hour second digit
	led_buffer[1] = hour % 10; //extract hour first digit
	led_buffer[2] = minute / 10; //extract minute second digit
	led_buffer[3] = minute % 10; //extract minute first digit
}
	...
	switch(index){
		case 0:
			...
			for(uint8_t i = 0; i < 8; i++){
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8 << i, (maxtrix_buffer[index] & (1 << i)) ? GPIO_PIN_RESET : GPIO_PIN_SET);
			}
